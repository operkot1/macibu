...
## 13. `traffic_signs` — справочник дорожных знаков (T-109, первый срез)
...
- **Откуда:** `MK noteikumi Nr. 279 «Ceļu satiksmes noteikumi», Pielikums
  ...
  rādītāji, papildzīmes). В этом срезе заполнена только одна категория —
  `"priority"` (знаки 201–209, «priekšrocības zīmes»), 9 из 9 знаков этой
  категории. Названия/значения свёрены по трём независимым источникам
  (credoautoprieks.lv, поисковый результат со ссылкой на likumi.lv,
  gross.lv) — при расхождении по знакам 203/204/205 (какой знак означает
  «справа», «слева», «с обеих сторон») решающей стала не текстовая сверка,
  а прямой геометрический анализ координат официальной векторной графики
  (см. ниже) — gross.lv оказался единственным источником с ошибкой в этой
  тройке, не использован для этих трёх записей. Остальные 7 категорий —
...
- **Изображения:** `image` — SVG из Wikimedia Commons
  (`Latvia_road_sign_{N}.svg`), прямая калька официального дизайна знака.
  Латвийские официальные дорожные знаки не защищены авторским правом —
  Autortiesību likums, 6. pants («Neaizsargātie darbi») прямо исключает
  государственные апробированные символы/знаки; подтверждено пометкой о
  лицензии на странице файла в Wikimedia Commons. Правильность номера
  знака для 203/204/205 (у кого ветка справа, у кого слева, у кого с
  обеих сторон) проверена не только по тексту источников, а прямым
  разбором координат SVG-path — у 203 перекладина есть с обеих сторон
  вертикального столбика (символ «крест»), у 204 — только справа, у 205 —
  только слева.
