...
interface TrafficSign {
  ...
  category: "priority" | "mandatory" | "service";
  name_lv: string;
  name_ru: string;
  meaning_lv: string;
  meaning_ru: string;
...
- **Откуда:** `MK noteikumi Nr. 279 «Ceļu satiksmes noteikumi», Pielikums
  ...
  rādītāji, papildzīmes). Заполнены три категории: `"priority"` (знаки
  201–209, «priekšrocības zīmes», 9 из 9, T-109 первый срез),
  `"mandatory"` (знаки 401–427, «rīkojuma zīmes», 27 из 27, T-109 второй
  срез) и `"service"` (знаки 601–634, «servisa zīmes», 34 из 34, T-109
  третий срез). Названия/значения всех категорий свёрены по двум
  независимым текстовым источникам (credoautoprieks.lv, gross.lv для
  401–427 и 601–634; дополнительно поисковый результат со ссылкой на
  likumi.lv для 201–209) — при любом расхождении между источниками или
  простом текстовом совпадении решающей проверкой был не текст, а прямой
  геометрический анализ координат официальной векторной графики и/или
  визуальный рендер (см. ниже). Остальные 5 категорий — будущие срезы,
  каждый со своим планированием (по решению пользователя,
