...
- **Откуда:** `MK noteikumi Nr. 279 «Ceļu satiksmes noteikumi», Pielikums
  ...
  rādītāji, papildzīmes). Заполнены шесть категорий: `"priority"` (знаки
  ...
  «brīdinājuma zīmes», 43 из 43, T-109 пятый срез) и `"direction"` —
  **единственная неполная категория**: 16 из 53 официальных номеров
  ...
  источникам (credoautoprieks.lv, gross.lv для 101–143, 301–334, 401–427
  и 601–634; дополнительно поисковый результат со ссылкой на likumi.lv
  для 201–209) — при любом расхождении между источниками или простом
  текстовом совпадении решающей проверкой был не текст, а прямой
  геометрический анализ координат официальной векторной графики и/или
  визуальный рендер (см. ниже). Остальные 2 категории — будущие срезы,
...
- **Изображения:** `image` — SVG из Wikimedia Commons
  ...
  - 701–753 (шестой срез, **частичный**): при попытке получить полный
    ...
    префиксам: 740 = красный фон, префикс «A» (валсts galvenais ceļš);
