...
### 2.2. Первый реальный прогон на GitHub Actions (T-006c, сентябрь 2026) —
два открытия, невидимых до появления удалённого репозитория

До T-006c/T-005 у `npm run check` никогда не было по-настоящему чистого
окружения: локальный Lighthouse всегда падал на несвязанный с проектом
`spawn Unknown system error -86` (песочница macOS этой машины, см. §1
задокументированного паттерна выше по сессии), а локальный `.astro/`
кеш существовал с самого первого `npm run dev`, скрывая зависимость от
`astro sync`. Оба факта оставались невидимыми, пока не появился реальный
удалённый CI-раннер (GitHub Actions, `ubuntu-latest`, настоящий Chrome
без песочничных ограничений):

1. **`tsc --noEmit` падал с `Cannot find module 'astro:content'`** на
   чистом чекауте — типы Content Collections генерирует `astro sync`,
   раньше вызывавшийся только неявно как часть `astro dev`/`astro build`.
   Исправлено: `"typecheck": "astro sync && tsc --noEmit"` (T-005).
2. **Lighthouse реально запустился и один раз реально провалил
   `total-blocking-time`** (204мс против порога 200мс) при
   `numberOfRuns: 1` — единичный замер оказался достаточно шумным, чтобы
   один и тот же коммит без изменений в коде на повторном прогоне прошёл
   чисто. Порог (200мс) не менялся — `numberOfRuns` поднят до 3 (T-006c),
   `lhci` по умолчанию агрегирует медианой при `numberOfRuns > 1`.

Оба открытия — прямое следствие того, что `npm run check` наконец
прогнался в среде, для которой он изначально писался (реальный CI, не
локальная песочница с известным несвязанным багом Chrome-спауна).
Branch protection (T-006c) живьём проверена тестовым PR с намеренно
сломанным линтом (`--no-verify` для одного тестового коммита, PR закрыт
и ветка удалена сразу после проверки) — CI провалился на ожидаемой
ошибке, `gh pr merge` явно отклонён политикой ветки
